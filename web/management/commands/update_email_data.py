...
LICENCE_REVOKED_BODY = f"""Dear [[EXPORTER_NAME]],

Your licence(s) [[LICENCE_NUMBER]] has been revoked.
...
A new mailshot has been published. You can view it from your workbasket in apply for an import licence or export certificate.
[Link to case in apply for an import licence or export certificate]
...
STOP_CASE_BODY = f"""Dear [[APPLICANT_NAME]],

Processing on apply for an import licence case reference [[CASE_REFERENCE]] has been stopped.
...
EMAIL_CONTENT = [
    (
        "IMA_APP_UPDATE",
        "[[CASE_REFERENCE]] Request for updates to your application",
    ...
    (
        "CA_BEIS_EMAIL",
        "[[CASE_REFERENCE]] Good manufacturing practice (GMP) application enquiry",
        BEIS_EMAIL_BODY,
    ),
    ("CASE_REOPEN", "[[CASE_REFERENCE]] Application processing resumed", CASE_REOPEN_BODY),
    (
        "CERTIFICATE_REVOKE",
        "[[CERTIFICATE_REFERENCES]] Certificate(s) revoked",
        CERTIFICATE_REVOKED_BODY,
    ),
    (
        "IMA_CONSTAB_EMAIL",
    ...
    ("CA_RFI_EMAIL", "[[CASE_REFERENCE]] Request for more information", CA_RFI_BODY),
    ("CA_HSE_EMAIL", "[[CASE_REFERENCE]] Biocidal product enquiry", HSE_BODY),
    ("IAR_RFI_EMAIL", "[[REQUEST_REFERENCE]] Request for more information", IAR_RFI_BODY),
    ("LICENCE_REVOKE", "[[LICENCE_NUMBER]] Licence(s) revoked", LICENCE_REVOKED_BODY),
    ("PUBLISH_MAILSHOT", "New Mailshot", PUBLISH_MAILSHOT_BODY),
    ("RETRACT_MAILSHOT", "Retracted Mailshot", RETRACT_MAILSHOT_BODY),
    (
        "IMA_SANCTIONS_EMAIL",
        "[[CASE_REFERENCE]] Import sanctions and adhoc licence",
        SANCTIONS_BODY,
    ),
    ("STOP_CASE", "[[CASE_REFERENCE]] Case stopped", STOP_CASE_BODY),
]


class Command(BaseCommand):
    help = """Update email templates."""
