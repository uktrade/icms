...
class LoginForm(auth_forms.AuthenticationForm):
    ...
    def confirm_login_allowed(self, user):
        if user.account_status != User.ACTIVE:
            ...
            logger.warn("Failed login attempt", user=user, status=user.account_status)
...
class RegistrationForm(ModelForm):
    ...
    security_answer_repeat = CharField(
        required=True, label="Confirm Security Answer", widget=PasswordInput(render_value=True)
    )
