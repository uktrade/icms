...
class BaseTestDataView:
    ...
    def test_authentication_failure_hmrc_creds(self):
        sender = make_testing_hawk_sender("GET", self.url, api_type="hmrc", hash_content=False)
        response = self.client.get(self.url, HTTP_HAWK_AUTHENTICATION=sender.request_header)
        assert response.status_code == HTTPStatus.BAD_REQUEST

    def test_authetication(self):
        sender = make_testing_hawk_sender(
            "GET", self.url, api_type="data_workspace", hash_content=False
        )
        response = self.client.get(self.url, HTTP_HAWK_AUTHENTICATION=sender.request_header)
...
class TestMetaDataView(BaseTestDataView):
    ...
    def _setup(self, cw_client):
        ...
        self.url = reverse("data-workspace:metadata", kwargs={"version": "v0"})
