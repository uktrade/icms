...
from web.domains.user.forms import (PeopleFilter, PhoneNumberForm,
                                    UserDetailsUpdateForm, UserListFilter)
...
class UserListFilterTest(TestCase):
    def setUp(self):
        ...
        UserFactory(username='nathan.drake',
                    is_active=True,
    ...
    def test_account_status_filter(self):
        results = self.run_filter(
            {'status': [User.CANCELLED, User.SUSPENDED, User.BLOCKED]})
        self.assertEqual(results.count(), 3)
...
class PeopleFilterTest(TestCase):
    ...
    def create_user(self, username, first_name, last_name, organisation,
    ...
    def setUp(self):
        self.create_user(username='jane',
                         first_name='Jane',
                         last_name='Doe',
                         organisation='Doe Ltd.',
                         department='IT',
                         job_title='Director')
        self.create_user(username='john',
                         first_name='John',
                         last_name='Doe',
                         organisation='Doe Ltd.',
                         department='Finance',
                         job_title='Accountant')
        self.create_user(username='melkor',
                         first_name='Melkor',
                         last_name='Unknown',
...
class UserDetailsUpdateFormTest(TestCase):
    ...
    def test_security_answer_validation(self):
        form = UserDetailsUpdateForm(data={
            'security_answer': 'Hello',
            'security_answer_repeat': 'Hi'
        })
...
class PhoneNumberFormTest(TestCase):
    def phone_number(self, number):
        return {
            ...
            'type':
            random.choice([
                PhoneNumber.WORK, PhoneNumber.FAX, PhoneNumber.MOBILE,
                PhoneNumber.HOME, PhoneNumber.MINICOM
            ])
        }
