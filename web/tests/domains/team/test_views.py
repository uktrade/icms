...
class TeamEditViewTest(AuthTestCase):
    ...
    def test_add_member_saves_form_changes_to_session(self):
        ...
        self.client.post(
            self.url, {
                'action': 'add_member',
                'name': 'Super Team',
                'description': 'Super team description'
            })
    ...
    def test_add_member_saves_role_changes_to_session(self):
        ...
        self.client.post(self.url, {
            'action': 'add_member',
            f'role_members_{role.id}': [user.id, user2.id]
        })
        session = self.client.session
        role_members = session[f'team:{self.team.id}']['role_members'][str(
            role.id)]
    ...
    def test_add_people(self):
        ...
        response = self.client.post(self.url, {
            'action': 'add_people',
            'selected_items': [user.id, user2.id]
        })
        members = response.context_data['members']
        self.assertEqual(len(members), 2)
        self.assertTrue(user in members)
