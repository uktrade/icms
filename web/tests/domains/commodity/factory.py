...
class CommodityFactory(factory.django.DjangoModelFactory):
    ...
    start_datetime = fake.date_time_between(
        start_date="-2y", end_date="+1y", tzinfo=datetime.timezone.utc
    )
    end_datetime = factory.LazyAttribute(
        lambda c: fake.date_time_between(
            start_date=c.start_datetime, end_date="+2y", tzinfo=datetime.timezone.utc
        )
    )
...
class CommodityGroupFactory(factory.django.DjangoModelFactory):
    ...
    start_datetime = fake.date_time_between(
        start_date="-3y", end_date="+2y", tzinfo=datetime.timezone.utc
    )
    end_datetime = factory.LazyAttribute(
        lambda c: fake.date_time_between(
            start_date=c.start_datetime, end_date="+3y", tzinfo=datetime.timezone.utc
        )
    )
