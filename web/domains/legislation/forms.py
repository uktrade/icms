...
class ProductLegislationFilter(ModelSearchFilter):
    ...
    is_biocidal = ChoiceFilter(field_name='is_biocidal',
                               choices=((True, 'Yes'), (False, 'No')),
                               lookup_expr='exact',
                               label='Is Biocidal')

    is_biocidal_claim = ChoiceFilter(field_name='is_biocidal_claim',
                                     choices=((True, 'Yes'), (False, 'No')),
                                     lookup_expr='exact',
...
class ProductLegislationForm(ModelEditForm):
    class Meta:
        model = ProductLegislation
        fields = [
            'name', 'is_biocidal', 'is_biocidal_claim',
        ...
        help_texts = {
            'is_biocidal':
            'Product type numbers and active ingredients must be \
                entered by the applicant when biocidal legislation is selected',
            'is_eu_cosmetics_regulation':
            "A 'responsible person' statement \
