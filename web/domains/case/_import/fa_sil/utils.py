...
def _get_sil_errors(application: models.SILApplication) -> ApplicationErrors:
    ...
    if application.section_ni and not has_section_ni_goods:
        ...
        section_errors = PageErrors(
            page_name="Add Licence for Section Movements into Northern Ireland", url=url
        )
        section_errors.add(
            FieldError(
                field_name="Add Section Movements into Northern Ireland",
                messages=["At least one section 'Movements into Northern Ireland' must be added"],
            )
        )
        errors.add(section_errors)
