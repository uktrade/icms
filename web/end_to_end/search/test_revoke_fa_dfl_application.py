...
def revoke_fa_dfl_application(page: Page, dfl_id: int) -> None:
    ...
    #
    # Licence can take up to 90s to be generated. If link not on page wait 5 seconds and reload. Max 20 attempts.
    utils.click_or_wait_and_reload(
        page,
        locator_str=f'[data-test-id="search-results-row-{dfl_id}"]',
        role_data={"role": "link", "name": "Revoke Licence"},
        attempts=20,
    )
