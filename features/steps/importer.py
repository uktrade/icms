...
def enters_importer_address(context, address):
    context.browser.find_element_by_css_selector(".new-form-row:last-child textarea").send_keys(
        address
    )
...
def check_importer_in_list(context, name):
    assert utils.find_element_by_text(
        context, name, "a[@role = 'model-link']"
    ), "importer {name} is not in the list"
...
def importer_name_reads(context, expected_name):
    name_field = context.browser.find_element_by_css_selector('form [data-label="Name"]')
    assert (
        name_field.text == expected_name
    ), f'expecting importer name to read "{expected_name}" but got "{name_field.text}"'
...
def importer_details_read(context):
    for field, value in context.table:
        ...
        assert (
            value == found_value
        ), f'expecting field "{field}" to be "{value}" but got "{found_value}"'
...
def importer_edit_form_reads(context):
    for field, value in context.table:
        ...
        if field == "Type" or field == "Region origin":
            found_value = (
                Select(context.browser.find_element_by_css_selector(id))
...
def importer_offices_read(context):
    ...
    field_map = {"Address": 1, "Post Code": 2, "ROI": 3}  # field name to table column mapping
...
def importer_agents_read(context):
    ...
    field_map = {"Importer Name / Importer Entity Type": 1, "Agent Type": 2, "Address": 3}
